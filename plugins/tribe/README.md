...
It works in three modes: **forge a roadmap** of many idea cards, **run a campaign** over approved cards, and **brainstorm together** — the owner brings ONE problem ("let's brainstorm together"), the Shaman grounds the owner's claims first, agrees a high-level solution (guardrails, ratchet, ledger, how to verify, do / don't), records the ratified decisions in the idea card, has a planning-only Warchief write the spec and plan and reviews them by grounding, then briefs and guides the owner's new execution session itself. The long form is Mode 3 of `agents/shaman.md`; the short form ships as the global CLAUDE.md snippet `claude-md/shaman-brainstorm-together.md`, so it applies in every session in every project.
