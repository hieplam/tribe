...
# --- Step 4: G6 — the reproducible half (project-directory listing). The viewer screenshot is the
# other half of G6: the runner serves the consolidated viewer on 127.0.0.1:4321 while it runs, so
# the page is reachable during the run (R13 corrected the earlier "headless runner cannot hold a
# foreground server" claim). The screenshot is captured from that viewer by the Warchief and
# embedded below as 2026-09-18-campaign-supervisor-e2e-viewer.png. ---------------------------
...
EVIDENCE="$REPO_ROOT/docs/superpowers/evidence/2026-09-18-campaign-supervisor-e2e.md"
{
  ...
  printf 'committed so every PR link resolves from the repo itself. The BEFORE/AFTER machine sections\n'
  printf 'below are emitted verbatim from that run; the G6 viewer screenshot and the "verify-shipped —\n'
  printf 'precisely what ran" note are Warchief-added post-run corrections (card ruling R13), and are\n'
  printf 'the only hand-added parts.\n\n'
  ...
  printf '### Step 4 — G6 viewer screenshot (R13)\n\n'
  printf 'The runner serves the consolidated campaign viewer on `127.0.0.1:4321` while it runs, so the\n'
  printf 'live viewer page is reachable during the run — no separate foreground server is needed. The\n'
  printf 'project page for this run'"'"'s campaign, captured from that viewer by the Warchief:\n\n'
  printf '![Campaign viewer — the project page listing this run'"'"'s ruling and closing sessions](2026-09-18-campaign-supervisor-e2e-viewer.png)\n\n'

  printf '### `supervisor/final-report.md`\n\n```\n%s\n```\n\n' "$(cat_or_missing "$HOME_DIR/supervisor/final-report.md")"

  printf '## verify-shipped — precisely what ran (R13.4)\n\n'
  printf 'R11 item 4 (skill **name resolution**) is met: the closing transcript shows\n'
  printf '`Skill{skill: verify-shipped}` resolving ("Launching skill: verify-shipped", never "Unknown\n'
  printf 'skill"). It does **not** by itself prove `verify-shipped.sh` ran to a script-emitted verdict —\n'
  printf 'a **BLOCKED** in `final-report.md` above is the closing model'"'"'s own judgment about the\n'
  printf 'synthetic PR, not a value the verify-shipped script returned. This evidence must not be read\n'
  printf 'as "verify-shipped returned BLOCKED".\n'
} > "$EVIDENCE"
if [[ -s "$EVIDENCE" ]]; then
