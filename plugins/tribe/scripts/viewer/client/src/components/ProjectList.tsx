...
export interface ProjectListProps {
  ...
  /** In-app navigation owned by `App`; each row links to its project's `/p/<dir>` route. */
...
function ProjectRow({ project, active, onNavigate }: { project: Project; active: boolean; onNavigate?: (route: ClientRoute) => void }) {
  return (
    <li>
      <RouteLink
        route={{ kind: 'project', projectDir: project.dir }}
...
export function ProjectList({ projects, activeProjectDir = null, onNavigate }: ProjectListProps) {
  return (
    <ul className="project-list">
      {projects.map((project) => (
        <ProjectRow key={project.dir} project={project} active={project.dir === activeProjectDir} onNavigate={onNavigate} />
      ))}
    </ul>
  );
}
